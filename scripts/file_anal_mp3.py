...
# filefolder=r
rootDir = r"Z:\NDHA\NDHA\manual_queries"
for dirName, subdirList, fileList in os.walk(rootDir):
        # print('Found directory: %s' % subdirList, fileList)  
    if "Magic" in os.path.join(dirName):
        for fi in os.listdir(os.path.join(dirName)):
            # print(dirt, fi)
            file_list=[]
            randoms=[]
            if fi.endswith('mp3') or fi.endswith('jpg'):
            ...
            for fl in file_list:
                ...
                with exiftool.ExifTool() as et:
                    ...
                    # if not album_name:
                    #     album_name=""
                    artist=et.get_tag("ID3:Artist", file_path)
                    if not artist:
                        artist=""
                    # date=et.get_tag("Vorbis:Date", file_path)
                    # if not date:
                    #     date=""
                    track_title=et.get_tag("ID3:Title", file_path)
                    if not track_title:
                        track_title=""
                    track_number=et.get_tag("ID3:Track", file_path)
                    # if not track_number:
                    #     track_number=""
                    # try:
                    ...
                    # if not MD5:
                    #     MD5=""
                    print(folder, artist, track_number, track_title, mime_type, file_path)
